...
rng(123);
nugget_test = 0:100:1000;
range_test = 10000:1000:25000;
sill_test = 1250:250:2500;
model_test= [0, 1];
...
cv_nugget = results(1, 2);
cv_range = results(1, 3);
cv_sill = results(1, 4);
cv_model_type = results(1, 5);

% Best model parameters
test_cv = spherical_model(h, cv_sill, cv_range, cv_nugget);

% Manual best fit
test_manual = exponential_model(h, 1800, 20000, 50);
manual_sill = 1800; 
manual_range = 20000;
manual_nugget = 50;
manual_model_type = 1;
writematrix([1, 2, 3, 4; manual_nugget, manual_range, manual_sill, manual_model_type], '../../data/man_variogram_params.csv');
...
%%% CV FIT VARIOGRAM KRIGING

[zp_cv, ze_cv] = johnson_ordinary_kriging(plot_res, Matp, cv_sill, cv_sill, cv_range, cv_nugget, cv_model_type);

zpcv_out= reshape(zp_cv,ny,nx);
zecv_out  =reshape (ze_cv,ny,nx);
...
title("Manual Variogram Kriging Prediction");

subplot(2, 1, 2);
man_krig_plot = imagesc(X2, Y2, zpcv_out); hold on
set(gca,'YDir','normal');
set(gca,'color',[0.5, 0.5, 0.5]);
set(gca,'XTick',[])
set(gca,'YTick',[])
colormap(rwb)
caxis([-max(abs(min(zp_cv)), abs(max(zp_cv))), max(abs(min(zp_cv)), abs(max(zp_cv)))]);
ylabel("Y (m)");
xlabel("X (m)");
z = colorbar;
set(get(z,'label'),'string',"Predicted Residual (AGB Mg ha^{-1})");
...
set(get(z,'label'),'FontSize',12);
plot(test_plot_res(:, 1), test_plot_res(:, 2), '.', 'Color', "#000000")
title("CV Variogram Kriging Prediction");
hold off;
...
set(get(z,'label'),'FontSize',12);
plot(test_plot_res(:, 1), test_plot_res(:, 2), '.', 'Color', "#000000")
title("Manual Variogram Kriging Variance");


subplot(2, 1, 2);
man_var_plot = imagesc(X2, Y2, zecv_out); hold on
set(gca,'YDir','normal');
set(gca,'color',[0.5, 0.5, 0.5]);
set(gca,'XTick',[])
set(gca,'YTick',[])
...
set(get(z,'label'),'FontSize',12);
plot(test_plot_res(:, 1), test_plot_res(:, 2), '.', 'Color', "#000000")
title("CV Variogram Kriging Variance");
hold off;
...
[cvpred_res, cvpred_err] = johnson_ordinary_kriging(plot_res, test_plot_res(:, [1, 2]), cv_sill, cv_sill, cv_range, cv_nugget, cv_model_type);
...
function [results] = grid_search_variogram(data, params)
    ...
    for i = 1:size(params, 1)
        param_rmse(i, 1) = k_fold_cv(data, 10, params(i, 1), params(i, 2), params(i, 3), params(i, 4));
    end
